...
int main(int argc, char *argv[]) {
	...
	std::string animFile = "topology.xml";  // Name of file for animation output
...
p2p.EnableAsciiAll (asciiTraceHelper.CreateFileStream("Elastic.tr"));
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      std::cout << "  Throughput: " << i->second.rxBytes * 8.0 / (simTime) / 1000 / 1000  << " Mbps\n";
			// myfile << i->second.rxBytes * 8.0 / (simTime) / 1000 / 1000 << "\n";
    }
		// myfile.close();
